...
> - some：用于检测数组中的元素是否满足指定条件。会依次执行数组的每个元素：如果有一个元素满足条件，则表达式返回 true，剩余的元素不会再执行检测。如果没有满足条件的元素，则返回 false。
> - every：用于检测数组所有元素是否都符合指定条件，如果有一个元素不满足，则返回 false，且剩余的元素不会再检测。如果所有元素都符合，则返回 true。
> - reduce：接收一个函数作为累加器，数组中的每个值(从左到右)开始缩减，最终计算为一个值。
> - find：返回通过测试(函数内判断)的数组的第一个元素的值。数组中的每一个元素都调用一次函数执行：当数组中的元素再测试条件时返回 true 时，find()返回符合条件的元素，之后的值不会再调用执行函数。如果没有符合条件的元素返回 undefined。
> - splice：用于添加或删除数组中的元素。这种方法会改变原始数组。

```javascript
// 移除数组的第三个元素，并在数组第三个位置添加新元素。
var fruits = ["Banana", "Orange", "Apple", "Mango"];
fruits.splice(2, 1, "Lemon", "Kiwi"); // 第一个参数是数组的索引，第二个参数是删除的个数，从第三个参数后的是添加的元素
// fruits 输出结果：Banana,Orange,Lemon,Kiwi,Mango
```

> - slice：提取字符串/数组的某个部分，并以新的字符串/数组返回被提取的部分
